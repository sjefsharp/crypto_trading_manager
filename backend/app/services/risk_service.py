...
def calculate_position_risk(position: Dict[str, Any], portfolio_value: float, portfolio_balance: float) -> Dict[str, Any]:
    ...
    position_size_percentage = (current_value / portfolio_value * 100) if portfolio_value > 0 else 0
    
    # Unrealized P&L percentage
    unrealized_pnl_percentage = (unrealized_pnl / position_cost * 100) if position_cost > 0 else 0
...
def calculate_portfolio_risk(portfolio: Dict[str, Any], market_data: Dict[str, float]) -> Dict[str, Any]:
    ...
    for position in positions:
        ...
        if current_price > 0:
            ...
            risk_data = calculate_position_risk(position_with_price, total_value, total_value)
...
def check_order_risk(order: Dict[str, Any], portfolio: Dict[str, Any], market_price: float) -> Dict[str, Any]:
    ...
    for position in existing_positions:
        if position.get("market") == market:
            ...
            current_exposure = (position_value / total_value * 100) if total_value > 0 else 0
            break
...
def calculate_stop_loss_price(entry_price: float, side: str, risk_percentage: float = 5.0) -> float:
...
def calculate_take_profit_price(entry_price: float, side: str, profit_ratio: float = 2.0, risk_percentage: float = 5.0) -> float:
...
def check_drawdown_limits(portfolio_history: List[Dict[str, Any]], max_drawdown: float = 20.0) -> Dict[str, Any]:
    ...
    current_drawdown = ((peak_value - current_value) / peak_value * 100) if peak_value > 0 else 0.0
...
def generate_risk_recommendations(risk_analysis: Dict[str, Any]) -> List[str]:
    ...
    if high_risk_positions > 0:
        recommendations.append(f"Review {high_risk_positions} high-risk positions for potential reduction")
    
    if max_position_risk > 30:
        recommendations.append("Largest position exceeds 30% of portfolio - consider diversifying")
    
    total_exposure = risk_analysis.get("total_exposure", 0.0)
    if total_exposure > 80:
        recommendations.append("Portfolio is highly concentrated - consider adding more positions")
    
    return recommendations


def validate_order_risk(order: Dict[str, Any], portfolio_value: float, risk_limits: Dict[str, Any]) -> Dict[str, Any]:
    """Validate if an order meets risk management criteria"""
    order_value = order.get("amount", 0.0) * order.get("price", 0.0)
    position_size_percentage = (order_value / portfolio_value * 100) if portfolio_value > 0 else 0
    ...
    if position_size_percentage > max_position_size:
        approved = False
        reasons.append(f"High risk: position size {position_size_percentage:.1f}% exceeds maximum {max_position_size}%")
    
    if order_value > max_order_value:
        approved = False
        reasons.append(f"High risk: order value {order_value:.2f} exceeds maximum {max_order_value}")
