...
class BitvavoAPI:
    ...
    def __init__(self, api_key: str = None, api_secret: str = None):
        ...
        if not self.api_key or not self.api_secret:
            if 'pytest' not in sys.modules:  # Only show warning in production
                print("⚠️ Bitvavo API credentials not found. Some features will be limited.")
    ...
    async def _make_request(self, method: str, endpoint: str, params: Dict = None, 
        ...
        if not self.session:
            raise RuntimeError("API session not initialized. Use 'async with BitvavoAPI() as api:' pattern")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return {"error": error_json.get("error", error_data), "status_code": response.status_code}
                except:
    ...
    async def get_candles(self, market: str, interval: str = "1h", limit: int = 24) -> List[List]:
    ...
    async def place_order(self, market: str, side: str, order_type: str, 
        ...
        if not trading_mode_service.is_live_trading():
            raise Exception("Live trading not enabled. Check trading mode configuration.")
        ...
        for key, value in kwargs.items():
            if value is not None:
                api_order_data[key] = str(value) if isinstance(value, (int, float)) else value
    ...
    async def get_trades(self, market: str = None, limit: int = 100) -> List[Dict[str, Any]]:
        """Get trade history"""
        if not self.api_key or not self.api_secret:
            raise ValueError("API credentials not configured")
...
def calculate_take_profit(price: float, side: str, take_profit_percentage: float) -> float:
...
def format_bitvavo_response(response: Dict[str, Any], message: str, 
...
class TradingHelpers:
    ...
    @staticmethod
    async def calculate_position_size(api: BitvavoAPI, market: str, risk_amount: float, 
    ...
    @staticmethod
    async def place_stop_loss_order(api: BitvavoAPI, market: str, side: str, 
