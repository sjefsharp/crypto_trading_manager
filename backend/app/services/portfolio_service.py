...
def calculate_portfolio_performance(portfolio_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for position in positions:
        ...
        position_perf = {
            ...
            "percentage_change": ((current_price - avg_price) / avg_price * 100) if avg_price > 0 else 0.0
        }
    ...
    if initial_balance > 0:
        total_return_percentage = ((current_balance - initial_balance) / initial_balance) * 100
...
def update_portfolio_positions(existing_positions: List[Dict], new_trade: Dict[str, Any]) -> List[Dict]:
    ...
    for position in existing_positions:
        if position.get("symbol") == symbol:
            ...
            if side == "buy":
                ...
                new_avg_price = ((existing_amount * existing_avg_price) + (amount * price)) / new_amount
...
def get_rebalance_suggestions(portfolio: Dict[str, Any], target_allocation: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    for position in positions:
        ...
        current_allocation[symbol] = market_value / total_value if total_value > 0 else 0.0
