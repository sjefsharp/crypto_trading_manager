...
async def store_api_key(
    ...
    try:
        ...
        if not request.api_key or not request.api_secret:
            raise HTTPException(
                status_code=400, detail="Both API key and secret are required"
            )

        if len(request.api_key) < 10 or len(request.api_secret) < 10:
            raise HTTPException(
                status_code=400, detail="API key and secret seem too short"
            )
        ...
        existing_key = (
            db.query(APIKey)
            .filter(APIKey.user_id == user_id, APIKey.exchange == exchange.lower())
            .first()
        )
...
async def get_api_keys_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving API key status: {str(e)}"
        )
...
async def delete_api_key(
    ...
    try:
        ...
        if not api_key:
            raise HTTPException(
                status_code=404, detail=f"No active API key found for {exchange}"
            )
...
async def test_api_key(
    ...
    try:
        ...
        if not api_key_record:
            raise HTTPException(
                status_code=404, detail=f"No API key configured for {exchange}"
            )
