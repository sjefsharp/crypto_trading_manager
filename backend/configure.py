...
from app.config.secure_config import setup_initial_config, get_secure_config
...
def show_config_status():
    ...
    print(f"✅ Configuration file: {'Found' if config.config_exists() else 'Not found'}")
    print(f"🔑 Bitvavo API: {'✓ Configured' if validation['bitvavo'] else '✗ Missing'}")
    print(f"📈 FRED API: {'✓ Configured' if validation['fred'] else '✗ Not configured (optional)'}")
    print(f"📊 BLS API: {'✓ Configured' if validation['bls'] else '✗ Not configured (optional)'}")
    print(f"🎯 Overall Status: {'✅ Ready' if validation['valid'] else '⚠️ Needs attention'}")
...
def delete_config():
    ...
    confirm = input("⚠️ This will permanently delete your API configuration. Continue? (y/N): ")
    if confirm.lower() == 'y':
