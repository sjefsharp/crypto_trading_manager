...
from app.models.database_models import Base, User, Portfolio, Trade, Position
...
class TestDatabaseModels:
    ...
    def test_user_model_creation(self, test_session):
        ...
        # Retrieve user
        retrieved_user = test_session.query(User).filter(User.username == "testuser").first()
    ...
    def test_portfolio_model_creation(self, test_session):
        ...
        # Retrieve portfolio
        retrieved_portfolio = test_session.query(Portfolio).filter(Portfolio.name == "My Crypto Portfolio").first()
    ...
    def test_trade_model_creation(self, test_session):
        ...
        user = User(username="testuser", email="test@example.com", hashed_password="hash123")
        test_session.add(user)
        test_session.commit()
        
        portfolio = Portfolio(user_id=user.id, name="Test Portfolio", description="Test")
        test_session.add(portfolio)
        test_session.commit()
        ...
        # Retrieve trade
        retrieved_trade = test_session.query(Trade).filter(Trade.order_id == "test-order-123").first()
    ...
    def test_position_model_creation(self, test_session):
        ...
        # Retrieve position
        retrieved_position = test_session.query(Position).filter(Position.symbol == "BTC").first()
    ...
    def test_user_unique_constraints(self, test_session):
        ...
        # Create first user
        user1 = User(username="testuser", email="test@example.com", hashed_password="hash123")
        test_session.add(user1)
        test_session.commit()
        
        # Try to create second user with same username
        user2 = User(username="testuser", email="different@example.com", hashed_password="hash456")
        ...
        user3 = User(username="differentuser", email="test@example.com", hashed_password="hash789")
